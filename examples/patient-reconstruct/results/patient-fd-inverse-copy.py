...
parser.add_argument('-f', '--fix_init', type=int, choices=[0, 1], default=1,
                    required=False, help='Whether to run the fixed-init calibration')
...
if args.fix_init == 1:
    ...
    print("Calibrating model for given initialization")

    result = fd_pde.calibrate_model_fixinit(
        obs=torch.as_tensor(tumor2 / tumor2.max(), device=device),
        init=torch.as_tensor(tumor1 / tumor1.max(), device=device),
        dt=0.001,
        max_iter=max_iter, method=method,
        verbose=True, message_period=max_iter//10)
    ...
    plot_dir = os.path.join(dir_path, "plots_fixinit", patient)
    os.makedirs(plot_dir, exist_ok=True)

    u, _, _ = fd_pde.solve_fixinit(
        init=torch.as_tensor(tumor1 / tumor1.max(), device=device),
    ...
    append_parameters_to_file(
        ...
        result['D'].item(), result['rho'].item(), [0.] * 3)
