...
class TumorInfiltraFD(TumorFixedFieldBase):
    ...
    def __init__(self, domain: VolumeDomain,
        ...
        self.parameters = torch.cat(
            [D.view(-1), rho.view(-1), init_params.view(-1)], dim=0)
    ...
    def _to_working_params(self, params: Tensor) -> Tensor:
        ...
        wkparams = (params - self.wkparams_min) / (self.wkparams_max - self.wkparams_min)
        return wkparams

    def _to_original_params(self, wkparams: Tensor) -> Tensor:
        """ Convert working parameters to their original scale"""
        params = wkparams * (self.wkparams_max - self.wkparams_min) + \
            self.wkparams_min
        return params
    ...
    def solve(self, dt: float = 0.01, t1: float = 1.,
        ...
        if self.nparam_init > 0:
            ...
            u = self.init_density(self.x_mesh, **self.init_other_params)
        if save_all:
    ...
    def _solve_with_grad(self, obs: Tensor, dt: float, t1: float,
                         D: Tensor, rho: Tensor, init_params: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    def _calibrate_loss_grads(
        ...
        u, _, grads = self.solve_with_grad(
            obs, dt, t1, self._D(params), self._rho(params), self._init_params(params))
        grads *= self.wkparams_max - self.wkparams_min
    ...
    def _solve_with_grad_multiscan(
            ...
            D: Tensor, rho: Tensor, init_params: Tensor) -> Tuple[Tensor, List[float], Tensor, Tensor]:
